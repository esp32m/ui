...
import { WidgetBox, useModuleState, NameValueList } from '../..';
import {
  Grid,
  IconButton,
  FormControlLabel,
...
import { DialogForm, FieldText, useDialogForm } from '../..';
...
export default ({ name, title }: IProps) => {
  ...
  const [hook, openSettings] = useDialogForm({
    ...
    onSubmit: async (hvac) => {
      await Backend.setState(name, { hvac });
    },
    validationSchema,
  });
  ...
  const list = [];
  list.push(['Master/Slave status', <>{masterStatusIcons} / {slaveStatusIcons}</>]);
