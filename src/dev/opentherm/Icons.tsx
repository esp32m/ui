import { styled } from '@mui/material/styles';
import { SVGProps } from 'react';

const defaultStyle = {
  width: 24,
  height: 24,
  paddingLeft:4,
};

export const Flame = styled((props: SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 611.999 611.999" {...props}>
    <g>
      <path
        d="M216.02,611.195c5.978,3.178,12.284-3.704,8.624-9.4c-19.866-30.919-38.678-82.947-8.706-149.952
            c49.982-111.737,80.396-169.609,80.396-169.609s16.177,67.536,60.029,127.585c42.205,57.793,65.306,130.478,28.064,191.029
            c-3.495,5.683,2.668,12.388,8.607,9.349c46.1-23.582,97.806-70.885,103.64-165.017c2.151-28.764-1.075-69.034-17.206-119.851
            c-20.741-64.406-46.239-94.459-60.992-107.365c-4.413-3.861-11.276-0.439-10.914,5.413c4.299,69.494-21.845,87.129-36.726,47.386
            c-5.943-15.874-9.409-43.33-9.409-76.766c0-55.665-16.15-112.967-51.755-159.531c-9.259-12.109-20.093-23.424-32.523-33.073
            c-4.5-3.494-11.023,0.018-10.611,5.7c2.734,37.736,0.257,145.885-94.624,275.089c-86.029,119.851-52.693,211.896-40.864,236.826
            C153.666,566.767,185.212,594.814,216.02,611.195z"
      />
    </g>
  </svg>
))(defaultStyle);

export const TapWater = styled((props: SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 502 502" {...props}>
        <g>
          <path
            d="M438.6,105.682h-30.26V93.875c0-5.523-4.478-10-10-10H295.453c-5.522,0-10,4.477-10,10v11.808H175.42
				c-67.282,0-122.02,54.738-122.02,122.02v108.851c0,5.523,4.478,10,10,10h106.241c5.522,0,10-4.477,10-10v-83.129
				c0-11.855,9.645-21.5,21.5-21.5h84.312v11.808c0,5.523,4.478,10,10,10H398.34c5.522,0,10-4.477,10-10v-11.808h30.26
				c5.522,0,10-4.477,10-10V115.682C448.6,110.159,444.122,105.682,438.6,105.682z M285.453,211.922h-84.311v0.001
				c-22.883,0-41.5,18.617-41.5,41.5v73.129H73.4v-98.851c0-56.254,45.766-102.02,102.02-102.02h110.033V211.922z M388.34,115.682
				v106.241v11.808h-82.887v-11.808V115.682v-11.808h82.887V115.682z M428.6,211.923h-20.26v-86.241h20.26V211.923z"
          />
          <path
            d="M210.984,160.91h22.366c5.522,0,10-4.477,10-10c0-5.523-4.478-10-10-10h-22.366c-5.522,0-10,4.477-10,10
				C200.984,156.433,205.462,160.91,210.984,160.91z"
          />
          <path
            d="M88.628,230.033c0,5.523,4.478,10,10,10c5.522,0,10-4.477,10-10c0-38.114,31.008-69.123,69.122-69.123
				c5.522,0,10-4.477,10-10c0-5.523-4.478-10-10-10C128.608,140.91,88.628,180.89,88.628,230.033z"
          />
          <path
            d="M279.797,20h58.218v34.733c0,5.523,4.478,10,10,10c5.522,0,10-4.477,10-10V20h58.219c5.522,0,10-4.477,10-10
				s-4.478-10-10-10H279.797c-5.522,0-10,4.477-10,10S274.274,20,279.797,20z"
          />
          <path
            d="M110.93,385.824c-3.438,0-6.636,1.767-8.466,4.677c-5.366,8.536-32.136,52.068-32.136,70.897
				c0,22.388,18.214,40.602,40.602,40.602s40.602-18.214,40.602-40.602c0-18.83-26.77-62.362-32.136-70.897
				C117.565,387.59,114.368,385.824,110.93,385.824z M110.93,482c-11.359,0-20.602-9.242-20.602-20.602
				c0-6.902,9.613-27.002,20.602-46.144c10.988,19.142,20.602,39.242,20.602,46.144C131.531,472.758,122.289,482,110.93,482z"
          />
        </g>
  </svg>
))(defaultStyle);

export const Heater = styled((props: SVGProps<SVGSVGElement>) => (
  <svg viewBox="0 0 60 60" {...props}>
      <g>
        <path
          d="M59,53.5h-7v-2.1c1.7-0.4,3-2,3-3.9v-2h3c0.6,0,1-0.4,1-1v-4c0-0.6-0.4-1-1-1h-3v-23h3c0.6,0,1-0.4,1-1v-4
			c0-0.6-0.4-1-1-1h-3v-2c0-2.2-1.8-4-4-4c-1.2,0-2.3,0.5-3,1.4c-0.7-0.8-1.8-1.4-3-1.4S42.7,5,42,5.9c-0.7-0.8-1.8-1.4-3-1.4
			S36.7,5,36,5.9c-0.7-0.8-1.8-1.4-3-1.4S30.7,5,30,5.9c-0.7-0.8-1.8-1.4-3-1.4S24.7,5,24,5.9c-0.7-0.8-1.8-1.4-3-1.4S18.7,5,18,5.9
			c-0.7-0.8-1.8-1.4-3-1.4S12.7,5,12,5.9c-0.7-0.8-1.8-1.4-3-1.4c-2.2,0-4,1.8-4,4v2H2c-0.6,0-1,0.4-1,1v4c0,0.6,0.4,1,1,1h3v23H2
			c-0.6,0-1,0.4-1,1v4c0,0.6,0.4,1,1,1h3v2c0,1.9,1.3,3.4,3,3.9v2.1H1c-0.6,0-1,0.4-1,1v4c0,0.6,0.4,1,1,1h58c0.6,0,1-0.4,1-1v-4
			C60,53.9,59.6,53.5,59,53.5z M57,41.5v2h-2v-2H57z M57,12.5v2h-2v-2H57z M51,6.5c1.1,0,2,0.9,2,2v3v4v25v4v3c0,1.1-0.9,2-2,2
			s-2-0.9-2-2v-39C49,7.4,49.9,6.5,51,6.5z M45,6.5c1.1,0,2,0.9,2,2v39c0,1.1-0.9,2-2,2s-2-0.9-2-2v-39C43,7.4,43.9,6.5,45,6.5z
			 M39,6.5c1.1,0,2,0.9,2,2v39c0,1.1-0.9,2-2,2s-2-0.9-2-2v-39C37,7.4,37.9,6.5,39,6.5z M33,6.5c1.1,0,2,0.9,2,2v39c0,1.1-0.9,2-2,2
			s-2-0.9-2-2v-39C31,7.4,31.9,6.5,33,6.5z M27,6.5c1.1,0,2,0.9,2,2v39c0,1.1-0.9,2-2,2s-2-0.9-2-2v-39C25,7.4,25.9,6.5,27,6.5z
			 M21,6.5c1.1,0,2,0.9,2,2v39c0,1.1-0.9,2-2,2s-2-0.9-2-2v-39C19,7.4,19.9,6.5,21,6.5z M15,6.5c1.1,0,2,0.9,2,2v39c0,1.1-0.9,2-2,2
			s-2-0.9-2-2v-39C13,7.4,13.9,6.5,15,6.5z M3,14.5v-2h2v2H3z M3,43.5v-2h2v2H3z M7,47.5v-3v-4v-25v-4v-3c0-1.1,0.9-2,2-2s2,0.9,2,2
			v39c0,1.1-0.9,2-2,2S7,48.6,7,47.5z M10,51.4c0.8-0.2,1.5-0.6,2-1.2c0.7,0.8,1.8,1.4,3,1.4s2.3-0.5,3-1.4c0.7,0.8,1.8,1.4,3,1.4
			s2.3-0.5,3-1.4c0.7,0.8,1.8,1.4,3,1.4s2.3-0.5,3-1.4c0.7,0.8,1.8,1.4,3,1.4s2.3-0.5,3-1.4c0.7,0.8,1.8,1.4,3,1.4s2.3-0.5,3-1.4
			c0.7,0.8,1.8,1.4,3,1.4s2.3-0.5,3-1.4c0.5,0.6,1.2,1,2,1.2v2.1H10V51.4z M58,57.5H2v-2h56V57.5z"
        />
        <rect x="26" y="0.5" width="2" height="2" />
        <rect x="32" y="0.5" width="2" height="2" />
        <rect x="38" y="0.5" width="2" height="2" />
        <rect x="44" y="0.5" width="2" height="2" />
        <rect x="50" y="0.5" width="2" height="2" />
        <rect x="20" y="0.5" width="2" height="2" />
        <rect x="14" y="0.5" width="2" height="2" />
        <rect x="8" y="0.5" width="2" height="2" />
    </g>
  </svg>
))(defaultStyle);
